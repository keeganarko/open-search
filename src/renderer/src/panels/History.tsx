...
export default function History({ onClose }: { onClose: () => void }): JSX.Element {
  ...
  useEffect(() => {
    const t = setTimeout(() => { void window.voyager.history.search(q, 300).then(setRows) }, 120)
      ...
      actions={
        <>
          ...
          <button className="btn danger" onClick={async () => {
            await window.voyager.history.clear()
            setRows(await window.voyager.history.search(q, 300))
          }}>Clear all</button>
        </>
      }
    >
      <div className="desc" style={{ marginBottom: 12 }}>
        Full-text search across everything Voyager read. Excluded sites were never stored.
      </div>
      ...
      {rows.map((h) => (
        <div className="list-row" key={h.id}>
          <button className="main" style={{ background: 'none', textAlign: 'left' }}
            onClick={() => window.voyager.tabs.create({ url: h.url })}>
