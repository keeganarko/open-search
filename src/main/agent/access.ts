...
export async function authorizeContext(win: VoyagerWindow): Promise<void> {
  ...
  const result = await dialog.showMessageBox(win.window, {
    ...
    detail: 'Your request, allowed page text, tab titles, history, bookmarks, saved memory, and conversation may be sent to Anthropic. Excluded sites and paused page access remain blocked. Passwords are never included. Connector calls require separate approval.',
